...
namespace DAL
{
    ...
    {
        //用于保存 链接服务器的sql语句
        public static string ConnString = @"Server=DESKTOP-78BG1A5;Database=wujing;User ID=sa; Password=root";
        ...
        public static string userDataBasesql5 = @"select * from users ";
        ...
        public static string imageDataBasesql7 = @"select * from (select t.*,m.companyName,m.companyNumber from
                                           (select i.*,p.* from connect1 as c1 left join product as p on c1.productType=p.productType left join image as i on i.imageNumber=c1.imageNumber) as t,
                                           (select i.*,c.* from connect2 as c2 left join company as c on c2.companyNumber=c.companyNumber left join image as i on i.imageNumber=c2.imageNumber) as m 
                                           where t.imageNumber=m.imageNumber) s                                   
where imageNumber like @imageNumber and partsName like @partsName and productType like @productType and companyNumber like @companyNumber";
        ...
        public static string departmentDataBasesql5 = @"select * from department";


        //操作notices表的语句
        public static string noticesDataBasesql1 = @"insert into notices values (@noticeType, @noticeDepartment, @noticeContent, @noticeTime, @noticeFlag)";
        public static string noticesDataBasesql2 = @"delete FROM notices where EXISTS (select top 5 * FROM notices) ";
        public static string noticesDataBasesql3 = @"select top 2 * FROM (select * FROM notices where noticeType=@noticeType) t ORDER BY noticeId DESC  ";
        public static string noticesDataBasesql4 = @"update FROM notices set noticeFlag=@noticeFlag where noticeId=@noticeId";
